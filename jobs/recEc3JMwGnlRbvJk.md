---
title: "Project Tailwind, Call for Ambitious AI Safety Initiatives"
employer: "Coefficient Giving"
job_id: "recEc3JMwGnlRbvJk"
last_updated: "2026-09-12T05:32:30.594875Z"
posted_at: "2026-09-10T00:06:00Z"
status: ready
apply_url: "https://coefficientgiving.org/tailwind/"
areas:
  - "AI safety & policy"
---

# Project Tailwind, Call for Ambitious AI Safety Initiatives

**[Coefficient Giving](https://coefficientgiving.org/)**

<dl class="job-meta">
<dt>Location</dt><dd>Remote, Global, USA</dd>
<dt>Role type</dt><dd>Funding</dd>
<dt>Experience</dt><dd>Multiple experience levels</dd>
<dt>Degree</dt><dd>Undergraduate degree or less</dd>
</dl>

**Areas & skills:** AI safety & policy, Strategy

## Summary
- Funding for founders to start new organisations to solve critical problems in AI safety.
- Example initiatives to cover gaps in the ecosystem include training fault injections, AI decision-making tools, alignment moonshots, and a frontier capabilities observatory.
- 18-36 month runway typically provided.
- Pre-seed, seed, and scale funding provided.

<p class="apply-cta"><a href="https://coefficientgiving.org/tailwind/">Apply →</a></p>

---
*Posted 2026-09-10 · Listing synced 2026-09-12 · 80k job ID `recEc3JMwGnlRbvJk`*
