---
title: "Research Engineer, Takeoff Intel"
employer: "Anthropic"
job_id: "recK4AuSRBE9WbJB4"
last_updated: "2026-09-11T10:54:39.074084Z"
posted_at: "2026-09-10T00:00:00Z"
status: ready
apply_url: "https://job-boards.greenhouse.io/anthropic/jobs/5416882008"
areas:
  - "AI safety & policy"
---

# Research Engineer, Takeoff Intel

**[Anthropic](https://www.anthropic.com/)**

<dl class="job-meta">
<dt>Location</dt><dd>San Francisco Bay Area, USA, USA (Confirmed Visas)</dd>
<dt>Role type</dt><dd>Full-time</dd>
<dt>Experience</dt><dd>Junior (1-4 years experience)</dd>
<dt>Degree</dt><dd>Undergraduate degree or less</dd>
<dt>Salary</dt><dd>$350,000 – $850,000</dd>
</dl>

**Areas & skills:** AI safety & policy, Research, Software engineering

## Summary
- In this role, you'll design and operate evaluation and measurement instruments to assess AI recursive self-improvement.
- Build data pipelines that transform model outputs and telemetry into reliable, actionable metrics.
- Prototype instruments rapidly, validate their accuracy, and iterate on designs.
- Review and supervise AI-generated code as a routine part of your workflow.
- Collaborate with research scientists to define measurement priorities and contribute to internal and public reports.

<p class="apply-cta"><a href="https://job-boards.greenhouse.io/anthropic/jobs/5416882008">Apply →</a></p>

---
*Posted 2026-09-10 · Listing synced 2026-09-11 · 80k job ID `recK4AuSRBE9WbJB4`*
