---
title: "Staff Software Engineer, Data Pipelines"
employer: "Perimeter"
job_id: "receZ2f3xs5GlNJoj"
last_updated: "2026-09-06T05:33:00.857878Z"
posted_at: "2026-07-07T00:01:00Z"
status: ready
apply_url: "https://www.linkedin.com/jobs/view/4437183894/?alternateChannel=search&eBP=NON_CHARGEABLE_CHANNEL&trk=d_flagship3_search_srp_jobs&refId=5xhUbSAZBLjrZtmN833Zeg%3D%3D&trackingId=YL9py16%2B5Y8GRfH%2BTmZzgQ%3D%3D"
areas:
  - "Biosecurity & pandemic preparedness"
---

# Staff Software Engineer, Data Pipelines

**[Perimeter](https://www.perimeter.bio/)**

<dl class="job-meta">
<dt>Location</dt><dd>Boston metro area, USA</dd>
<dt>Role type</dt><dd>Full-time</dd>
<dt>Experience</dt><dd>Senior (10+ years experience)</dd>
<dt>Degree</dt><dd>Undergraduate degree or less</dd>
</dl>

**Areas & skills:** Biosecurity & pandemic preparedness, Software engineering

## Summary
- In this role, you'll architect and scale critical biosecurity data systems, leading design of reliable data pipelines and models.
- Lead architectural design and deployment of data warehouses, ETL/ELT pipelines, and data marts using AWS and Snowflake.
- Design and develop high-performance APIs and microservices in Python for the biosecurity platform.
- Own production-grade data workflows using AWS, Kubernetes, Airflow, and infrastructure-as-code tools.
- Lead cross-functional data projects and mentor engineers to translate strategic goals into robust data products.

<p class="apply-cta"><a href="https://www.linkedin.com/jobs/view/4437183894/?alternateChannel=search&amp;eBP=NON_CHARGEABLE_CHANNEL&amp;trk=d_flagship3_search_srp_jobs&amp;refId=5xhUbSAZBLjrZtmN833Zeg%3D%3D&amp;trackingId=YL9py16%2B5Y8GRfH%2BTmZzgQ%3D%3D">Apply →</a></p>

---
*Posted 2026-07-07 · Listing synced 2026-09-06 · 80k job ID `receZ2f3xs5GlNJoj`*
