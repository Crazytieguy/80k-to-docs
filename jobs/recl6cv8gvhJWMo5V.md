---
title: "Senior Software Engineer, AI Benchmarking"
employer: "SecureBio"
job_id: "recl6cv8gvhJWMo5V"
last_updated: "2026-08-20T05:32:30.842892Z"
posted_at: "2026-07-29T00:04:11Z"
status: ready
apply_url: "https://securebio.org/careers/senior-ai-benchmarking"
areas:
  - "AI safety & policy"
  - "Biosecurity & pandemic preparedness"
---

# Senior Software Engineer, AI Benchmarking

**[SecureBio](https://securebio.org/)**

<dl class="job-meta">
<dt>Location</dt><dd>San Francisco Bay Area, Boston metro area, USA</dd>
<dt>Role type</dt><dd>Full-time</dd>
<dt>Experience</dt><dd>Mid (5-9 years experience)</dd>
<dt>Degree</dt><dd>Undergraduate degree or less</dd>
<dt>Salary</dt><dd>$180,000 – $245,000</dd>
</dl>

**Areas & skills:** AI safety & policy, Biosecurity & pandemic preparedness, Software engineering

## Summary
- In this role, you'll build and scale biosecurity evaluations on frontier AI systems to measure dual-use biology risks and inform safety decisions.
- Perform pre-release assessments of frontier and open-source AI models using SecureBio's evaluation suite.
- Develop model adapters, troubleshoot unexpected agent behaviors, and ensure methodological rigor.
- Build and improve internal tools and cloud infrastructure for running capability assessments at scale.
- Contribute analyses to dashboards tracking AI biology capability trends.

<p class="apply-cta"><a href="https://securebio.org/careers/senior-ai-benchmarking">Apply →</a></p>

---
*Posted 2026-07-29 · Listing synced 2026-08-20 · 80k job ID `recl6cv8gvhJWMo5V`*
