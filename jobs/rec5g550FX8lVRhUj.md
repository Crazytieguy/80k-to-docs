---
title: "AI Architect, Research and Development"
employer: "Coalition for Epidemic Preparedness Innovations"
job_id: "rec5g550FX8lVRhUj"
last_updated: "2026-05-27T05:32:40.610912Z"
posted_at: "2026-05-20T00:01:00Z"
status: ready
apply_url: "https://cepi.my.salesforce-sites.com/careers/xcdrecruit__PositionDetails?id=a43P300000EZFCFIA5"
areas:
  - "Biosecurity & pandemic preparedness"
---

# AI Architect, Research and Development

**[Coalition for Epidemic Preparedness Innovations](https://cepi.net/)**

<dl class="job-meta">
<dt>Location</dt><dd>Oslo, Norway, London, UK, Washington, DC metro area, UK, USA, Norway, Europe (ex UK)</dd>
<dt>Role type</dt><dd>Full-time</dd>
<dt>Experience</dt><dd>Junior (1-4 years experience)</dd>
<dt>Degree</dt><dd>Doctoral degree</dd>
</dl>

**Areas & skills:** Biosecurity & pandemic preparedness, Software engineering

## Summary
- In this role, you'll support technical architecture and engineering for CEPI's Pandemic Preparedness Engine (PPX).
- Develop and maintain PPX technical designs, including architecture diagrams and implementation documentation.
- Build and test architecture components such as data pipelines, workflow orchestration, model services, and security controls.
- Apply federated patterns to enable cross-border collaboration while respecting data protection and sovereignty.
- Work with vaccine R&D stakeholders to capture requirements, benchmark AI models, and deliver reliable prototypes.

<p class="apply-cta"><a href="https://cepi.my.salesforce-sites.com/careers/xcdrecruit__PositionDetails?id=a43P300000EZFCFIA5">Apply →</a></p>

---
*Posted 2026-05-20 · Listing synced 2026-05-27 · 80k job ID `rec5g550FX8lVRhUj`*
