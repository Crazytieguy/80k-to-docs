---
title: "Member of Technical Staff, Design Engineer"
employer: "Valthos"
job_id: "recTJehTvOLGnijM6"
last_updated: "2026-08-02T05:31:46.566583Z"
posted_at: "2026-07-30T00:01:00Z"
status: ready
apply_url: "https://jobs.ashbyhq.com/valthos/9b7ea5d6-7900-4013-bfe7-2c28d90fd6d2"
areas:
  - "AI safety & policy"
  - "Biosecurity & pandemic preparedness"
---

# Member of Technical Staff, Design Engineer

**[Valthos](https://valthos.com/)**

<dl class="job-meta">
<dt>Location</dt><dd>Remote, Global</dd>
<dt>Role type</dt><dd>Full-time</dd>
<dt>Experience</dt><dd>Junior (1-4 years experience), Mid (5-9 years experience)</dd>
<dt>Degree</dt><dd>Undergraduate degree or less</dd>
</dl>

**Areas & skills:** AI safety & policy, Biosecurity & pandemic preparedness, Software engineering

## Summary
- In this role, you'll own the design and frontend implementation of core product experiences, from early concepts through production.
- Work with users, engineers, and biologists to understand complex workflows and translate them into clear, usable software.
- Design and build interfaces for data exploration, analytical workflows, alerts, and decision support.
- Rapidly prototype new concepts for user research, pilots, and customer discovery, refining through feedback.
- Establish and evolve the visual language, interaction patterns, and design system foundations.

<p class="apply-cta"><a href="https://jobs.ashbyhq.com/valthos/9b7ea5d6-7900-4013-bfe7-2c28d90fd6d2">Apply →</a></p>

---
*Posted 2026-07-30 · Listing synced 2026-08-02 · 80k job ID `recTJehTvOLGnijM6`*
