---
title: "Research Internship, Agentic AI, LLM Evaluation, and Trustworthy Systems"
employer: "Siemens"
job_id: "recCs6tciW94yfaKg"
last_updated: "2026-07-03T05:31:52.695525Z"
posted_at: "2026-07-01T00:00:00Z"
status: ready
apply_url: "https://jobs.siemens.com/en_US/externaljobs/JobDetail/510552?source=LinkedIn&sourceType=PREMIUM_POST_SITE"
areas:
  - "Career development: Technical"
---

# Research Internship, Agentic AI, LLM Evaluation, and Trustworthy Systems

**[Siemens](http://siemens.com/)**

<dl class="job-meta">
<dt>Location</dt><dd>Princeton, NJ, USA</dd>
<dt>Role type</dt><dd>Internship</dd>
<dt>Experience</dt><dd>Entry-level</dd>
<dt>Degree</dt><dd>Master's degree</dd>
<dt>Salary</dt><dd>$66,560 – $97,760</dd>
</dl>

**Areas & skills:** Career development: Technical, Research, Software engineering

## Summary
- In this role, you'll research and prototype V&V methods for multi-agent systems, emphasizing reliability, safety, and trustworthiness.
- Design evaluation harnesses and benchmarks for LLM agents including tool use, reasoning, and failure-mode analysis.
- Implement proof-of-concept prototypes in Python using modern AI frameworks and formal methods.
- Investigate verification strategies such as model checking, property testing, runtime monitoring, and guardrails.
- Collaborate with researchers to run experiments, analyze results, and document findings for publications.

<p class="apply-cta"><a href="https://jobs.siemens.com/en_US/externaljobs/JobDetail/510552?source=LinkedIn&amp;sourceType=PREMIUM_POST_SITE">Apply →</a></p>

---
*Posted 2026-07-01 · Listing synced 2026-07-03 · 80k job ID `recCs6tciW94yfaKg`*
